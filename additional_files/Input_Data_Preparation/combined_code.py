...
# Suppress warnings (especially useful for cfgrib)
warnings.filterwarnings('ignore')

# --- Configuration ---
SOURCE_DIR = Path("./hrrr")            # Original HRRR GRIB2 directory
ERA5_DIR = Path("./api_files")         # ERA5 daily NetCDF directory
OUTPUT_DIR = Path("./combined_nc")     # Final Combined NetCDF output directory
...
def get_hrrr_reference_info():
    """Finds a valid HRRR GRIB file to extract Coordinates."""
    grib_files = list(SOURCE_DIR.rglob("*.grib2"))
    if not grib_files:
        raise FileNotFoundError(f"No HRRR GRIB2 files found in {SOURCE_DIR}")
    
    # Open a single file just to get the grid coordinates
    ds = xr.open_dataset(
        grib_files[0], 
        engine='cfgrib', 
    ...
    return np.array(TARGET_LEVELS, dtype=float), lat, lon

def load_hour_raw_grib(file_path):
    """Loads a single GRIB file, filters, renames, and returns in-memory dataset."""
    try:
        ds = xr.open_dataset(
            file_path, 
            engine='cfgrib', 
        ...
        # Rename variables and coordinate
        rename_dict = {k: v for k, v in VAR_MAPPING_HRRR.items() if k in ds.data_vars}
        if 'isobaricInhPa' in ds.coords:
            rename_dict['isobaricInhPa'] = 'level'
        ds = ds.rename(rename_dict)
        
        # Keep only target variables
        vars_to_keep = [v for v in VAR_MAPPING_HRRR.values() if v in ds.data_vars]
        ds = ds[vars_to_keep]
        
        # Load to memory so we can safely close the file hook
        ds = ds.load()
        return ds
...
def process_hour(date_str, hour_idx, hrrr_interp_info):
    """
    Processes a SINGLE HOUR by dynamically reading the GRIB and ERA5 files,
    then writes the combined output to a NetCDF file.
    """
    try:
        ...
        hrrr_file = SOURCE_DIR / date_str / f"hrrr.t{hour_idx:02d}z.wrfprsf00.grib2"
        era5_file = ERA5_DIR / f"api_daily_{date_str}.nc"
        
        # 1. Load HRRR Data (Directly from GRIB to memory)
        ds_hrrr = None
        if hrrr_file.exists():
            ds_hrrr = load_hour_raw_grib(hrrr_file)
        ...
        with netCDF4.Dataset(output_path, 'w', format='NETCDF4') as nc:
            ...
            invert_lat = e_lats[0] > e_lats[-1]
            if invert_lat:
                e_lats = e_lats[::-1]
            
            # 4. Process Variables Layer by Layer
            for var in vars_to_proc:
                hrrr_avail = ds_hrrr is not None and var in ds_hrrr.variables
                     ...
                     for lev_idx, _ in enumerate(target_levels):
                         ...
                         if hrrr_vol is not None:
                             try:
                                 ...
                                 # Triangulation using precomputed Delaunay
...
def main():
    ...
    # Pre-compute Simplex Indices for Target Grid (Speeds up workers massively)
    ...
    print(f"Scanning {SOURCE_DIR} for tasks...")
    tasks = []
    ...
    # Check SLURM CPUS mapping
    MAX_WORKERS = int(os.environ.get('SLURM_CPUS_PER_TASK', 50)) 
    if MAX_WORKERS <= 1: 
        MAX_WORKERS = 60 # Default to a high count if the env variable isn't properly picked up
